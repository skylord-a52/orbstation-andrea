...
/datum/grand_side_effect/spell/trigger(potency, turf/ritual_location, mob/invoker)
	if (sound)
...
/datum/grand_side_effect/spell/scramble_turfs
	abstract = FALSE
	spell_path = /datum/action/cooldown/spell/spacetime_dist
	sound = 'sound/magic/timeparadox2.ogg'

/datum/grand_side_effect/spell/scramble_turfs/trigger(potency, turf/ritual_location, mob/invoker)
	duration = LERP((10 SECONDS), (15 SECONDS), potency/GRAND_RITUAL_FINALE_COUNT)
	return ..()
...
/datum/grand_side_effect/create_anomalies/trigger(potency, turf/ritual_location, mob/invoker)
	...
	while(to_create > 0)
		...
		new create_path(pick(anomaly_positions), rand(150, 300), FALSE)
		to_create--
