	...
	switch(adjusted_pressure)
		...
		if(HAZARD_HIGH_PRESSURE to INFINITY)
			if(!HAS_TRAIT(H, TRAIT_RESISTHIGHPRESSURE))
				H.adjustBruteLoss(min(((adjusted_pressure / HAZARD_HIGH_PRESSURE) - 1) * PRESSURE_DAMAGE_COEFFICIENT, MAX_HIGH_PRESSURE_DAMAGE) * H.physiology.pressure_mod * delta_time, required_status = BODYTYPE_ORGANIC)
		...
		// Very low pressure, show an alert and take damage
			...
			if(HAS_TRAIT(H, TRAIT_RESISTLOWPRESSURE))
				...
				H.adjustBruteLoss(LOW_PRESSURE_DAMAGE * H.physiology.pressure_mod * delta_time, required_status = BODYTYPE_ORGANIC)
